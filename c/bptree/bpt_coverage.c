...
int main(void)
{
        struct bplus_tree *tree = bplus_tree_init("./data/coverage.index", 512);
        exec_file("testcase", tree);
        show_running_info();
        /* test range search */
        bplus_tree_get_range(tree, 1000, 2000);
        bplus_tree_get_range(tree, 2000, 1000);
        bplus_tree_deinit(tree);

        tree = bplus_tree_init("./data/coverage.index", 128);
        bplus_tree_deinit(tree);
